...
def preprocess_accidents(accidents_df):
    ...
    return (
        accidents_df.select(
            "ACCIDENT_ID", "DT_ACCDN", "LOC_LAT", "LOC_LONG", "HEURE_ACCDN"
        )
...
def get_negative_samples(
    ...
    road_features_df = get_road_features_df(spark, road_df=road_df, use_cache=use_cache)
    ...
    negative_samples = negative_samples.withColumn(
        "sample_id", monotonically_increasing_id()
    )
    ...
    negative_sample_weather = get_weather_information(negative_samples, weather_df)
    negative_samples = negative_samples.join(negative_sample_weather, "sample_id")
...
def get_positive_samples(
    spark, road_df=None, weather_df=None, year_limit=None, use_cache=True, limit=None
):
    ...
    weather_df = weather_df or get_weather_df(spark, accident_df)
    road_features_df = get_road_features_df(
        spark, road_df=road_df, use_cache=use_cache
    ...
    positive_samples = (
        accident_df.join(accident_weather, "sample_id")
...
def get_weather_information(samples, weather_df):
    ...
    weather_cols = list(
        ...
        - set(["station_id", "hour", "station_lat", "station_long", "date"])
    )
    weighted_weather_cols = [
        when(isnan(col(c)), 0)
        .otherwise(col("inv_dist_to_station") * col(c))
        .alias(c + "_weighted")
        for c in weather_cols
    ]
    coeffs_weather_cols = [
        when(isnan(col(c)), 0).otherwise(col("inv_dist_to_station")).alias(c + "_coeff")
        for c in weather_cols
    ]
    ...
    return (
        samples.join(weather_df, on=["date", "hour"])
        .withColumn(
            "distance_inter",
            distance_intermediate_formula(
                "loc_lat", "loc_long", "station_lat", "station_long"
            ),
        )
        .withColumn("dist_to_station", distance_measure() * (6371 * 2 * 1000))
        .withColumn("inv_dist_to_station", 1 / col("dist_to_station") ** p)
        .select("sample_id", *(weighted_weather_cols + coeffs_weather_cols))
        .groupBy("sample_id")
...
def remove_positive_samples_from_negative_samples(neg_samples, pos_samples):
    pos_samples_to_remove = pos_samples.select(
        "date", "hour", "street_id", lit(1).alias("exists")
    )
    neg_samples = (
        neg_samples.join(
            pos_samples_to_remove, ["date", "hour", "street_id"], "left_outer"
        )
...
def get_dataset_df(spark, pos_samples, neg_samples):
    ...
    street_level_index = get_street_categories_index(
        spark, "street_level", train_pos, roads
    )
    street_type_index = get_street_categories_index(
        spark, "street_type", train_pos, roads
    )

    def prepare_dataset(va, pos_samples, neg_samples):
        dataset = pos_samples.union(neg_samples)
        dataset = (
            dataset.join(accident_count, "street_id")
            .join(street_level_index, "street_level")
            .join(street_type_index, "street_type")
            .drop("street_level", "street_type")
        )
        return va.transform(dataset).select(
            "sample_id", "street_id", "date", "hour", "features", "label"
        )

    va = VectorAssembler(
        outputCol="features", inputCols=features_col, handleInvalid="keep"
    )
