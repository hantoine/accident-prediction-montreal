...
def get_station_weather_month(station_id, year, month):
    ...
    """
    url = (
        f"http://climate.weather.gc.ca/climate_data/bulk_data_e.html?"
    ...
    df.loc[df["Temp (°C)"].notna(), "risky_weather"] = (
        df["Weather"].astype(str).str.contains("snow", regex=True, flags=re.IGNORECASE)
    )
...
def get_weather_station_weather_df(spark, stations_id):
    ...
    get_station_weather_month_udf = udf(
        get_station_weather_month,
        ArrayType(
            StructType(
                [
    ...
    month_per_year_df = spark.createDataFrame(
        zip(
            range(1, 13),
        ),
        ["month"],
    ...
    df = (
        stations_months_df.repartition(200, "year", "month")
        .withColumn(
            "weather", get_station_weather_month_udf("station_id", "year", "month")
        )
    ...
    df = df.withColumn(
        "risky_weather", weighted_average("risky_weather", window, offsets, weights)
    )
...
def get_weather_station_coords(station_id):
    ...
    url = (
        f"http://climate.weather.gc.ca/climate_data/hourly_data_e.html?"
    ...
    long = (
        coords_div.find_all("div", recursive=False)[1]
