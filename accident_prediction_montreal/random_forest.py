...
def balanced_random_forest_tuning(train_samples):
    rf = RandomForestClassifier(
        labelCol="label", featuresCol="features", cacheNodeIds=True, weightCol="weight"
    )
    ru = RandomUnderSampler().setIndexCol("id")
    cw = ClassWeighter()
...
def compute_precision_recall(predictions, threshold):
    ...
    true_positive = (
        ...
        .filter((col("label") == 1.0) & (prob_positive_udf("probability") > threshold))
        .count()
    )
    false_positive = (
        predictions.select("label", "prediction")
        .filter((col("label") == 0.0) & (prob_positive_udf("probability") > threshold))
        .count()
    )
    true_negative = (
        predictions.select("label", "prediction")
...
def compute_precision_recall_graph_slow(predictions, n_points):
    ...
    graph = pd.DataFrame(
        [gen_row(t) for t in space], columns=["Threshold", "Precision", "Recall"]
    )
...
def compute_threshold_dependent_metrics(spark, predictions, n_points):
    ...
    id_buckets = spark.createDataFrame(
        zip(
            range(-1, n_points),
        ),
        ["id_bucket"],
    ...
    return (
        predictions.select(
            "label", floor(prob_positive("probability") * n_points).alias("id_bucket")
        )
        .groupBy("label", "id_bucket")
        .count()
        .join(id_buckets.crossJoin(label), ["id_bucket", "label"], "outer")
        .na.fill(0)
        .withColumn("count_negatives", sum("count").over(inf_cumulative_window))
        .withColumn("count_positives", sum("count").over(sup_cumulative_window))
        .groupBy("id_bucket")
        .pivot("label", [0, 1])
        .sum("count_negatives", "count_positives")
        .select(
            ((col("id_bucket") + 1) / n_points).alias("Threshold"),
        ...
        .withColumn(
            "Precision",
            col("true_positive") / (col("true_positive") + col("false_positive")),
        )
        .withColumn(
            "Recall",
            when(
                col("true_positive") != 0,
                col("true_positive") / (col("true_positive") + col("false_negative")),
            ).otherwise(0.0),
        )
        .withColumn(
            "False positive rate",
            col("false_positive") / (col("false_positive") + col("true_negative")),
        )
        .withColumn(
            "Accuracy",
